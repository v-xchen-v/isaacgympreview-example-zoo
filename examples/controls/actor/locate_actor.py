...
# gym.set_dof_target_position(env0, cart_dof_handle0, 0) # cpu mode
# locate the dof by (env, actor_handle, dof_name) or (env, actor_handle, dof_index) and get the index
# Ref: https://docs.robotsfan.com/isaacgym/programming/tensors.html#degrees-of-freedom
cart_dof_index = gym.get_actor_dof_index(env0, cartpole0, 0, gymapi.DOMAIN_SIM)
cart_dof_index = gym.find_actor_dof_index(env0, cart_dof_handle0, 'slider_to_cart', gymapi.DOMAIN_SIM)


# locate the actor by (env, actor_handle) and get the index
cart_index = gym.find_actor_index(env0, 'cartpole', gymapi.DOMAIN_SIM)
gym.set_dof_position_target_tensor_indexed(sim, 
                                  gymtorch.unwrap_tensor(torch.tensor([0.0, 0.25*math.pi], dtype=torch.float32)), 
                                  gymtorch.unwrap_tensor(torch.tensor([cart_index]).to(torch.int32)),
                                  1) # gpu mode
